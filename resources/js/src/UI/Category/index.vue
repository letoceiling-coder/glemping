<template>
                ...
                <div class="col-12">
                    <div class="card card-info ">
                        <div class="mb-3 flex gab-5 ai-center">
                            <button @click="resetForm();this.modalShow('#modal-slug-category')" id="fa-plus"
                                    class="btn btn-xs btn-default bg-gradient-black" >
                                <i class="fa-solid fa-plus"></i>
                            </button>
                            <label class="m-0" for="fa-plus">Создать категорию</label>
                        ...
                        <div class="row flex column">
                            ...
                            >
                                <Children v-for="category in categories" :key="category.id" :category="category"
                                          :dataId="category.id"
                                          :form="form"></Children>
                            </ul>
    ...
    <div class="modal fade" id="modal-slug-category">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{{
                            actions.find(item => item.id == this.storage.category.action).name
                        }}</h4>
...
export default {
    ...
    methods: {
        ...
        sortsCategory() {
            this.categories = this.categories.sort((a, b) => (a.sort > b.sort) ? 1 : -1);
        ...
        async updateSort(ids) {
            ...
            await axios.post('/api/v1/categories/update-sort',
                {
                    ids: ids,
                }
            ).then(data => {
        ...
        async send() {
            ...
            if (this.storage.category.action == 'edit' || this.storage.category.action == 'delete') {
                url += `/${this.form.id}`
            }
            console.log(url)
