<template>
  <span v-if="!btn" v-bind="$attrs" @click="togglePicker">
     <i class="vue3-icon-picker" :class="modelValue"></i>

  </span>
  <span v-if="btn" v-bind="$attrs" @click="togglePicker">
     <div type="button" class="btn btn-outline-dark " >
        ...
        <div class="aim-modal--content">
          ...
          <span class="aim-modal--header-logo-title">
            ...
            <div class="aim-modal--body">
                ...
                <div class="aim-modal--icon-preview-wrap">
                    <div class="aim-modal--icon-search">
                        <input v-model="filterText" placeholder="Поиск" />
            ...
            <div class="aim-modal--footer">
                <button class="aim-insert-icon-button" @click="insert">Выбрать</button>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted() {
        ...
        if (this.modelValue == ''){
            this.$emit('update:modelValue', 'fas fa-search')

        }
        this.eventBus.on('clickIconPicker', (args) => {
